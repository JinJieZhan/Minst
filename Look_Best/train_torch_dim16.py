...
model = ViT(CLSisUsed=False,PostionisUsed=True).to(DEVICE)

# Attempt to load pretrained model
try:
    model.load_state_dict(torch.load('Torch_flagTrue_CLSisUsedFalse_PostionisUsedTrue_torch_dim16.pth', map_location=DEVICE))
...
try:
    ...
    for epoch in range(EPOCHS):
        ...
        torch.save(model.state_dict(), 'Torch_flagTrue_CLSisUsedFalse_PostionisUsedTrue_torch_dim16.pth')
...
# Test Loss: 0.117959, Accuracy: 0.964100